...
- 🖼️ **图片题 OCR优先 + 单链降级**：本地 PP-OCRv6 优先免费提取 → 未达标时按 GLM → 豆包 Mini → Qwen VL Flash → Qwen3.7-Plus → 豆包 Pro 2.1 降级
- 🤖 **9 个模型多层故障转移**：支持 DeepSeek、GLM、豆包、Qwen，任一失败自动降级
- 🆓 **本地 OCR（PP-OCRv6）**：文字截图优先本地识别，满足阈值则零成本跳过云端 API
- 📝 **智能答案后处理**：
...
4. **双击 `scripts\一键启动.bat`**：
   - 自动检查 Python / 创建 venv / 装依赖 / 读 key / 启动服务
    - 完成后弹出浏览器，**按 `docs/使用教程.html` 装 OCS 油猴脚本**
...
本项目仅供**学习研究**目的，旨在展示 AI API 调用、答案后处理等技术实践。

- 使用本项目的**用户**应自行遵守所在学校/平台的相关规定，作者不对使用者的行为负责
- 请勿将本项目用于学术作弊、违规代考等违反学术诚信的场景
